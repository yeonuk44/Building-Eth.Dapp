...
- 트러플 : 가장 유명한 개발 프레임워크로 솔리디티 컨트랙트의 구축, 테스트, 패키징 및 배포를 단순화하는 데 주로 중점을 둡니다. 가장 큰 장점으로는 마이그레이션입니다. 마이그레이션이란 간단히 컨트랙트를 배포하는 스크립트와 설정을 관리하는 방식입니다. 저도 트러플로 개발을 진행할 예정입니다.
